...
internal `startTermweaveSidecar` bootstrap, then supplies `() => <App />`. All fd 0 handling,
renderer construction, raw stdout, signals, and shutdown behavior live in
`termweave/sidecar-runtime.tsx`.

The thin WebView entry is `termweave/host/webview-entry.ts`; orchestration lives in
`webview-host.ts`. Xterm construction, sidecar transport/window lifecycle, monitor presentation, and
the complete CRT effect group have separate owners. Startup still creates the fixed xterm surface,
always activates monitor/noise presentation, attempts same-canvas WebGL CRT postprocessing, launches
the packaged OpenTUI sidecar, and preserves the stock renderer as the transactional fallback.
...
`app/screens.ts` remains the only screen registry. `ScreenKey` is inferred from its `animation`,
`picture`, and `plain` keys. `app/store.ts` creates the typed SDK navigation instance and exposes it
with durable application state and actions. No router, history, provider, binding table, or
compatibility layer is present.
...
3. Replace `app.icon.png` and copy application-owned images into `app/assets/`; the icon is not an
...
6. Replace v1 SDK imports with `#termweave`. The exact runtime surface is `PixelRenderer`,
   `getTermweaveConfig`, and `createScreenNavigation`; the public types are `PixelRendererProps`,
   `TermweaveConfig`, and `ScreenNavigation`.
